...
def compute_tail_heaviness(image, use_sobel=True, display=False, detect_fire=False):
    if detect_fire:
        if display:
            original_img = image 
        ...
        if display:
            ...
            plt.figtext(0.5, 0.01, f"sigma1: {sigma1:.4f}", ha='center', fontsize=14, color='blue')
            plt.show()
